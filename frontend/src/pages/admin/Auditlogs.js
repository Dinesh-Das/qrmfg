...
const Auditlogs = () => {
  ...
  const fetchLogs = async () => {
    setLoading(true);
    try {
      const res = await axios.get('/qrmfg/api/v1/audit/logs');
      setLogs(res.data);
    } catch (err) {
  ...
  const fetchFilteredLogs = async (filters) => {
    setLoading(true);
    let url = '/qrmfg/api/v1/audit/logs';
    if (filters) {
      if (filters.userId) url = `/qrmfg/api/v1/audit/logs/user/${filters.userId}`;
      else if (filters.action) url = `/qrmfg/api/v1/audit/logs/action/${filters.action}`;
      else if (filters.entityType) url = `/qrmfg/api/v1/audit/logs/entity/${filters.entityType}`;
      else if (filters.severity) url = `/qrmfg/api/v1/audit/logs/severity/${filters.severity}`;
      else if (filters.dateRange && filters.dateRange.length === 2) {
        const [start, end] = filters.dateRange;
        url = `/qrmfg/api/v1/audit/logs/date?start=${start.toISOString()}&end=${end.toISOString()}`;
      }
    }
    try {
