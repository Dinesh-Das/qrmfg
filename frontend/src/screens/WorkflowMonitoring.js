...
const WorkflowMonitoring = () => {
  ...
  const fetchDashboardData = async () => {
    setLoading(true);
    try {
      const response = await apiRequest('/admin/monitoring/dashboard');
  ...
  const fetchBottlenecks = async () => {
    ...
    try {
      const response = await apiRequest('/admin/monitoring/bottlenecks');
  ...
  const handleExportAuditLogs = async () => {
    ...
    try {
      ...
      const response = await fetch(`${process.env.REACT_APP_API_BASE_URL || 'http://localhost:8081/qrmfg/api/v1'}${url}`, {
        headers: {
