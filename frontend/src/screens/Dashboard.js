...
const Dashboard = () => {
  ...
  useEffect(() => {
    setLoading(true);
    Promise.all([
      apiRequest('/reports/analytics/user-stats'),
      apiRequest('/reports/analytics/role-distribution'),
      apiRequest('/reports/analytics/activity-timeline'),
    ]).then(([userStats, roleDistribution, activityTimeline]) => {
      setUserStats(userStats);
      setRoleDist(Object.entries(roleDistribution).map(([role, count]) => ({ role, count })));
      setActivityTimeline(Object.entries(activityTimeline).map(([date, count]) => ({ date, count })));
